...
use crate::consteval::conv::get_conversion;

use super::{
    apply_conversion, conv::Convert, ConstEvalError, Instance, LiteralInstance, MatInner,
    MatInstance, Ty, VecInner, VecInstance,
};

use wgsl_parse::syntax::*;

pub trait Compwise: Clone + Sized {
    fn compwise_unary_mut<F>(&mut self, f: F) -> Result<(), ConstEvalError>
    where
        F: Fn(LiteralInstance) -> Result<LiteralInstance, ConstEvalError>;
    fn compwise_binary_mut<F>(&mut self, rhs: &Self, f: F) -> Result<(), ConstEvalError>
    where
        F: Fn(LiteralInstance, LiteralInstance) -> Result<LiteralInstance, ConstEvalError>;
    fn compwise_unary<F>(&self, f: F) -> Result<Self, ConstEvalError>
    where
        F: Fn(LiteralInstance) -> Result<LiteralInstance, ConstEvalError>,
    {
    ...
    fn compwise_binary<F>(&self, rhs: &Self, f: F) -> Result<Self, ConstEvalError>
    where
        F: Fn(LiteralInstance, LiteralInstance) -> Result<LiteralInstance, ConstEvalError>,
    {
        let mut res = self.clone();
...
impl LiteralInstance {
    fn op_neg(&self) -> Result<LiteralInstance, ConstEvalError> {
        match self {
            LiteralInstance::AbstractInt(lhs) => Ok(LiteralInstance::AbstractInt(-lhs)),
            LiteralInstance::AbstractFloat(lhs) => Ok(LiteralInstance::AbstractFloat(-lhs)),
            LiteralInstance::I32(lhs) => Ok(LiteralInstance::I32(-lhs)),
            LiteralInstance::F32(lhs) => Ok(LiteralInstance::F32(-lhs)),
            LiteralInstance::F16(lhs) => Ok(LiteralInstance::F16(-lhs)),
            _ => Err(ConstEvalError::InvalidUnary(
                UnaryOperator::Negation,
                self.ty(),
            )),
        }
    }
    fn op_add(&self, rhs: &Self) -> Result<LiteralInstance, ConstEvalError> {
        match apply_conversion(self, rhs)? {
            both!(Self::AbstractInt, lhs, rhs) => Ok((lhs + rhs).into()),
            both!(Self::AbstractFloat, lhs, rhs) => Ok((lhs + rhs).into()),
            both!(Self::I32, lhs, rhs) => Ok((lhs + rhs).into()),
            both!(Self::U32, lhs, rhs) => Ok((lhs + rhs).into()),
            both!(Self::F32, lhs, rhs) => Ok((lhs + rhs).into()),
            both!(Self::F16, lhs, rhs) => Ok((lhs + rhs).into()),
    ...
    fn op_sub(&self, rhs: &Self) -> Result<LiteralInstance, ConstEvalError> {
        match apply_conversion(self, rhs)? {
            both!(Self::AbstractInt, lhs, rhs) => Ok((lhs - rhs).into()),
            both!(Self::AbstractFloat, lhs, rhs) => Ok((lhs - rhs).into()),
            both!(Self::I32, lhs, rhs) => Ok((lhs - rhs).into()),
    ...
    fn op_mul(&self, rhs: &Self) -> Result<LiteralInstance, ConstEvalError> {
        match apply_conversion(self, rhs)? {
            both!(Self::AbstractInt, lhs, rhs) => Ok((lhs * rhs).into()),
            both!(Self::AbstractFloat, lhs, rhs) => Ok((lhs * rhs).into()),
            both!(Self::I32, lhs, rhs) => Ok((lhs * rhs).into()),
    ...
    fn op_div(&self, rhs: &Self) -> Result<LiteralInstance, ConstEvalError> {
        match apply_conversion(self, rhs)? {
            both!(Self::AbstractInt, lhs, rhs) => Ok((lhs / rhs).into()),
            both!(Self::AbstractFloat, lhs, rhs) => Ok((lhs / rhs).into()),
            both!(Self::I32, lhs, rhs) => Ok((lhs / rhs).into()),
    ...
    fn op_rem(&self, rhs: &Self) -> Result<LiteralInstance, ConstEvalError> {
        match apply_conversion(self, rhs)? {
            both!(Self::AbstractInt, lhs, rhs) => Ok((lhs % rhs).into()),
            both!(Self::AbstractFloat, lhs, rhs) => Ok((lhs % rhs).into()),
            both!(Self::I32, lhs, rhs) => Ok((lhs % rhs).into()),
...
impl Neg for LiteralInstance {
    type Output = Result<LiteralInstance, ConstEvalError>;
    fn neg(self) -> Result<LiteralInstance, ConstEvalError> {
        self.op_neg()
    }
}
...
impl<const N: usize> VecInner<N> {
    fn op_add_vec(&self, rhs: &Self) -> Result<Self, ConstEvalError> {
        let (lhs, rhs) = apply_conversion(self, rhs)?;
        lhs.compwise_binary(&rhs, |l, r| l + r)
    }
    fn op_sub_vec(&self, rhs: &Self) -> Result<Self, ConstEvalError> {
        let (lhs, rhs) = apply_conversion(self, rhs)?;
        lhs.compwise_binary(&rhs, |l, r| l - r)
    }
    fn op_mul_vec(&self, rhs: &Self) -> Result<Self, ConstEvalError> {
        let (lhs, rhs) = apply_conversion(self, rhs)?;
        lhs.compwise_binary(&rhs, |l, r| l * r)
    }
    fn op_div_vec(&self, rhs: &Self) -> Result<Self, ConstEvalError> {
        let (lhs, rhs) = apply_conversion(self, rhs)?;
        lhs.compwise_binary(&rhs, |l, r| l / r)
    }
    fn op_rem_vec(&self, rhs: &Self) -> Result<Self, ConstEvalError> {
        let (lhs, rhs) = apply_conversion(self, rhs)?;
        lhs.compwise_binary(&rhs, |l, r| l % r)
    }
    fn op_mul_scalar(&self, scalar: &LiteralInstance) -> Result<VecInner<N>, ConstEvalError> {
        let ty = get_conversion(&self.inner_ty(), &scalar.inner_ty()).ok_or_else(|| {
            ConstEvalError::InvalidBinary(BinaryOperator::Multiplication, self.ty(), scalar.ty())
        })?;
        let (vec, scalar) = (self.convert_inner_to(&ty)?, scalar.convert_to(&ty)?);

        vec.compwise_unary(|k| k * scalar)
    }
    fn op_mul_mat<const C: usize>(
        &self,
        rhs: &MatInner<C, N>,
    ) -> Result<VecInner<C>, ConstEvalError> {
        // TODO must be float
        let ty = get_conversion(&self.inner_ty(), &rhs.inner_ty()).ok_or_else(|| {
            ConstEvalError::InvalidBinary(BinaryOperator::Multiplication, self.ty(), rhs.ty())
        })?;
...
impl VecInstance {
    ...
    fn op_mul_scalar(&self, rhs: &LiteralInstance) -> Result<Self, ConstEvalError> {
        match self {
    ...
    fn op_mul_mat(&self, rhs: &MatInstance) -> Result<Self, ConstEvalError> {
        match (self, rhs) {
...
impl Neg for VecInstance {
    type Output = Result<VecInstance, ConstEvalError>;
...
impl<const C: usize, const R: usize> MatInner<C, R> {
    fn op_add_mat(&self, rhs: &Self) -> Result<Self, ConstEvalError> {
        let (lhs, rhs) = apply_conversion(self, rhs)?;
        lhs.compwise_binary(&rhs, |l, r| l + r)
    }

    fn op_sub_mat(&self, rhs: &Self) -> Result<Self, ConstEvalError> {
        let (lhs, rhs) = apply_conversion(self, rhs)?;
        lhs.compwise_binary(&rhs, |l, r| l - r)
    }

    fn op_mul_scalar(&self, rhs: &LiteralInstance) -> Result<MatInner<C, R>, ConstEvalError> {
        // TODO must be float
        let ty = get_conversion(&self.inner_ty(), &rhs.inner_ty()).ok_or_else(|| {
            ConstEvalError::InvalidBinary(BinaryOperator::Multiplication, self.ty(), rhs.ty())
        })?;
    ...
    fn op_mul_vec(&self, rhs: &VecInner<C>) -> Result<VecInner<R>, ConstEvalError> {
        // TODO must be float
        let ty = get_conversion(&self.inner_ty(), &rhs.inner_ty()).ok_or_else(|| {
            ConstEvalError::InvalidBinary(BinaryOperator::Multiplication, self.ty(), rhs.ty())
        })?;
    ...
    fn op_mul_mat<const K: usize>(
        &self,
        rhs: &MatInner<K, C>,
    ) -> Result<MatInner<K, R>, ConstEvalError> {
        // TODO must be float
        let ty = get_conversion(&self.inner_ty(), &rhs.inner_ty()).ok_or_else(|| {
            ConstEvalError::InvalidBinary(BinaryOperator::Multiplication, self.ty(), rhs.ty())
        })?;
...
impl MatInstance {
    fn op_add_mat(&self, rhs: &Self) -> Result<Self, ConstEvalError> {
        match (self, rhs) {
            ...
            (lhs @ _, rhs @ _) => Err(ConstEvalError::InvalidBinary(
                BinaryOperator::Addition,
                lhs.ty(),
                rhs.ty(),
            )),
    ...
    fn op_mul_scalar(&self, rhs: &LiteralInstance) -> Result<MatInstance, ConstEvalError> {
        match self {
    ...
    fn op_mul_vec(&self, rhs: &VecInstance) -> Result<VecInstance, ConstEvalError> {
        match (self, rhs) {
    ...
    pub fn op_mul_mat(&self, rhs: &Self) -> Result<MatInstance, ConstEvalError> {
        match (self, rhs) {
...
impl Add for &MatInstance {
    type Output = Result<MatInstance, ConstEvalError>;
    fn add(self, rhs: Self) -> Self::Output {
        self.op_add_mat(rhs)
    }
}
...
impl Neg for Instance {
    type Output = Result<Instance, ConstEvalError>;
...
impl LiteralInstance {
    pub fn op_eq(&self, rhs: &Self) -> Result<bool, ConstEvalError> {
        match apply_conversion(self, rhs)? {
    ...
    pub fn op_ne(&self, rhs: &Self) -> Result<bool, ConstEvalError> {
        match (self, rhs) {
            both!(Self::Bool, lhs, rhs) => Ok(lhs != rhs),
            both!(Self::AbstractInt, lhs, rhs) => Ok(lhs != rhs),
            both!(Self::AbstractFloat, lhs, rhs) => Ok(lhs != rhs),
    ...
    pub fn op_lt(&self, rhs: &Self) -> Result<bool, ConstEvalError> {
        match (self, rhs) {
            both!(Self::Bool, lhs, rhs) => Ok(lhs < rhs),
            both!(Self::AbstractInt, lhs, rhs) => Ok(lhs < rhs),
            both!(Self::AbstractFloat, lhs, rhs) => Ok(lhs < rhs),
    ...
    pub fn op_le(&self, rhs: &Self) -> Result<bool, ConstEvalError> {
        match (self, rhs) {
            both!(Self::Bool, lhs, rhs) => Ok(lhs <= rhs),
            both!(Self::AbstractInt, lhs, rhs) => Ok(lhs <= rhs),
            both!(Self::AbstractFloat, lhs, rhs) => Ok(lhs <= rhs),
    ...
    pub fn op_gt(&self, rhs: &Self) -> Result<bool, ConstEvalError> {
        match (self, rhs) {
            both!(Self::Bool, lhs, rhs) => Ok(lhs > rhs),
            both!(Self::AbstractInt, lhs, rhs) => Ok(lhs > rhs),
            both!(Self::AbstractFloat, lhs, rhs) => Ok(lhs > rhs),
    ...
    pub fn op_ge(&self, rhs: &Self) -> Result<bool, ConstEvalError> {
        match (self, rhs) {
            both!(Self::Bool, lhs, rhs) => Ok(lhs >= rhs),
            both!(Self::AbstractInt, lhs, rhs) => Ok(lhs >= rhs),
            both!(Self::AbstractFloat, lhs, rhs) => Ok(lhs >= rhs),
...
impl VecInstance {
    pub fn op_eq(&self, rhs: &Self) -> Result<VecInstance, ConstEvalError> {
        let (lhs, rhs) = apply_conversion(self, rhs)?;
        lhs.compwise_binary(&rhs, |a, b| a.op_eq(&b).map(Into::into))
    }
    pub fn op_ne(&self, rhs: &Self) -> Result<VecInstance, ConstEvalError> {
        let (lhs, rhs) = apply_conversion(self, rhs)?;
        lhs.compwise_binary(&rhs, |a, b| a.op_ne(&b).map(Into::into))
    }
    pub fn op_lt(&self, rhs: &Self) -> Result<VecInstance, ConstEvalError> {
        let (lhs, rhs) = apply_conversion(self, rhs)?;
        lhs.compwise_binary(&rhs, |a, b| a.op_lt(&b).map(Into::into))
    }
    pub fn op_le(&self, rhs: &Self) -> Result<VecInstance, ConstEvalError> {
        let (lhs, rhs) = apply_conversion(self, rhs)?;
        lhs.compwise_binary(&rhs, |a, b| a.op_le(&b).map(Into::into))
    }
    pub fn op_gt(&self, rhs: &Self) -> Result<VecInstance, ConstEvalError> {
        let (lhs, rhs) = apply_conversion(self, rhs)?;
        lhs.compwise_binary(&rhs, |a, b| a.op_gt(&b).map(Into::into))
    }
    pub fn op_ge(&self, rhs: &Self) -> Result<VecInstance, ConstEvalError> {
        let (lhs, rhs) = apply_conversion(self, rhs)?;
...
impl Instance {
    pub fn op_eq(&self, rhs: &Self) -> Result<Instance, ConstEvalError> {
        match (self, rhs) {
    ...
    pub fn op_ne(&self, rhs: &Self) -> Result<Instance, ConstEvalError> {
        match (self, rhs) {
            both!(Self::Literal, lhs, rhs) => lhs
                .op_ne(rhs)
                .map(|b| Self::Literal(LiteralInstance::Bool(b))),
    ...
    pub fn op_lt(&self, rhs: &Self) -> Result<Instance, ConstEvalError> {
        match (self, rhs) {
            both!(Self::Literal, lhs, rhs) => lhs
                .op_lt(rhs)
                .map(|b| Self::Literal(LiteralInstance::Bool(b))),
    ...
    pub fn op_le(&self, rhs: &Self) -> Result<Instance, ConstEvalError> {
        match (self, rhs) {
            both!(Self::Literal, lhs, rhs) => lhs
                .op_le(rhs)
                .map(|b| Self::Literal(LiteralInstance::Bool(b))),
    ...
    pub fn op_gt(&self, rhs: &Self) -> Result<Instance, ConstEvalError> {
        match (self, rhs) {
            both!(Self::Literal, lhs, rhs) => lhs
                .op_gt(rhs)
                .map(|b| Self::Literal(LiteralInstance::Bool(b))),
    ...
    pub fn op_ge(&self, rhs: &Self) -> Result<Instance, ConstEvalError> {
        match (self, rhs) {
            both!(Self::Literal, lhs, rhs) => lhs
                .op_ge(rhs)
                .map(|b| Self::Literal(LiteralInstance::Bool(b))),
